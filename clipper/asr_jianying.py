...
封装 VideoCaptioner jianying 引擎的 subprocess 调用。
"""
...
_log = get_logger("clipper.asr_jianying")

_ZH_EN_LANGS = {"auto", "zh", "en", "zh-CN", "en-US", "zh-Hans", "zh-TW"}


class JianyingBackend(ASRBackend):
    """剪映 ASR 后端,通过 VideoCaptioner Python API 调用。"""
    ...
    async def transcribe(
        ...
        def _run() -> tuple[bool, str, list[str]]:
            try:
                ...
                module = importlib.import_module("videocaptioner.core.asr.jianying")
                asr_class = module.JianYingASR
                ...
                result = asr.run()
                text = result.to_txt()
                _log.info("asr_success", engine="jianying")
                return True, text, []
            except Exception as e:
                import traceback

                full_err = traceback.format_exc()
                _log.warning("asr_failed", engine="jianying", error=full_err[:500])
                return False, "", [f"jianying 转写失败: {e}", f"完整错误链:\n{full_err}"]
        ...
        try:
            ...
            _log.warning("asr_timeout", engine="jianying", timeout=timeout)
            return False, "", [f"jianying 转写超时({timeout}s)"]
